...
.. container:: banner warmup
    ...
    Intuition - Adelie or Gentoo?

.. highlights::

   .. figure:: logisitc_regression_warmup.png


   Imagine you are a biologist wanting to assign a penguin to its correct species (**y**). 
   Assume that the only information available to you is the Culmen Length of the penguin in mm (**x**). 
...
Logistic regression is a classification method. Despite its name it is not a regression method.
...
**weight** (also called **coefficient**) of the single input feature. For more than one feature, we have a weight
for each feature:
...
To get the actual result, we need to *threshold* it to get the predicted class. The default threshold is 0.5:
